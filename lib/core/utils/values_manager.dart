import 'package:flutter_screenutil/flutter_screenutil.dart';

class AppMargin {
  static double m8 = 8.0.sp;
  static double m10 = 10.0.sp;
  static double m12 = 12.0.sp;
  static double m14 = 14.0.sp;
  static double m16 = 16.0.sp;
  static double m18 = 18.0.sp;
  static double m20 = 20.0.sp;
  static double m30 = 30.0.sp;
  static double m35 = 35.0.sp;
  static double m40 = 40.0.sp;
  static double m60 = 60.0.sp;
  static double m120 = 120.0.sp;
  static double m130 = 130.0.sp;
  static double m140 = 140.0.sp;
  static double m150 = 150.0.sp;
  static double m170 = 170.0.sp;
}

class AppPadding {
  static double p0 = 0.0.sp;
  static double p4 = 4.0.sp;
  static double p5 = 5.0.sp;
  static double p8 = 8.0.sp;
  static double p10 = 10.0.sp;
  static double p12 = 12.0.sp;
  static double p14 = 14.0.sp;
  static double p15 = 15.0.sp;
  static double p16 = 16.0.sp;
  static const double p16C = 16.0;
  static double p18 = 18.0.sp;
  static double p20 = 20.0.sp;
  static const double p20C = 20.0;
  static const double p27C = 27.0;
  static double p40 = 40.0.sp;
  static const double p30C = 30.0;
  static const double p40C = 40.0;
  static const double p50C = 50.0;
  static double p10Minus = -10.0.sp;
  static double p20Minus = -20.0.sp;
  static double p30Minus = -30.0.sp;
  static double p40Minus = -40.0.sp;
  static double p55Minus = -55.0.sp;
  static int p130Minus = -130;
}

class AppSize {
  static double s0 = 0.0.sp;
  static double s1 = 1.0.sp;
  static double s2 = 2.0.sp;
  static double s3 = 3.0.sp;
  static double s4 = 4.0.sp;
  static double s5 = 5.0.sp;
  static double s8 = 8.0.sp;
  static double s10 = 10.0.sp;
  static double s12 = 12.0.sp;
  static double s14 = 14.0.sp;
  static double s16 = 16.0.sp;
  static double s17 = 17.0.sp;
  static double s18 = 18.0.sp;
  static double s20 = 20.0.sp;
  static double s22 = 22.0.sp;
  static double s24 = 24.0.sp;
  static double s25 = 25.0.sp;
  static double s30 = 30.0.sp;
  static double s32 = 32.0.sp;
  static double s35 = 35.0.sp;
  static double s40 = 40.0.sp;
  static double s100 = 100.0.sp;
  static double s120 = 120.0.sp;
  static double s130 = 130.0.sp;
  static double s170 = 170.0.sp;
  static double s190 = 190.0.sp;
  static double s43 = 43.0.sp;
  static double s51 = 51.0.sp;
  static double s55 = 55.0.sp;
  static double s63 = 63.0.sp;
  static double s64 = 64.0.sp;
  static double s75 = 75.0.sp;
  static const double s14C = 14;
  static const double s26C = 26;
  static const int s250C = 250;
  static const double s200 = 200;
  static const double s220 = 220;
  static const double s300 = 300;
  static const double s500 = 500;
}
